...
public class CameraActivity extends Activity implements View.OnClickListener {
    ...
    //反面返回
    public final static int REQUEST_BACK = 0X14;
     ...
     * 跳转到拍照页面
     */
    public static void navToCamera(Context context, int type,int status) {
        ...
        if(type == TYPE_ID_CARD_FRONT){
            ...
            ((Activity) context).startActivityForResult(intent, REQUEST_BACK);
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (type) {
            case TYPE_ID_CARD_FRONT:
                if(status==1){
                    cropView.setImageResource(R.mipmap.take_ph);
                }else {
