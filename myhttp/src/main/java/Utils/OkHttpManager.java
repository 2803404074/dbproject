...
public class OkHttpManager {
     ...
     * 初始化RequestManager
     */
    private OkHttpManager(Context context, String baseUrl) {
        this.BASE_URL = baseUrl;
        System.out.println("++++++++++++++++++++++++++++++++++++传过来的token = ");
        //初始化OkHttpClient
     ...
     * @return
     */
    public static OkHttpManager getInstance(Context context, String baseUrl) {
        OkHttpManager inst = mInstance;
        if (inst == null) {
            synchronized (OkHttpManager.class) {
                inst = mInstance;
                if (inst == null) {
                    inst = new OkHttpManager(context.getApplicationContext(), baseUrl);
                    mInstance = inst;
                }
            }
        }
...
//                .addHeader("appVersion", "3.2.0");
     ...
     */
    public String requestSyn(String actionUrl, String token, int requestType, HashMap<String, String> paramsMap) {
        switch (requestType) {
            case TYPE_GET:
                requestGetBySyn(actionUrl, token, paramsMap);
                break;
            case TYPE_POST_JSON:
                requestPostBySyn(actionUrl, token, paramsMap);
                break;
            case TYPE_POST_FORM:
                return requestPostBySynWithForm(actionUrl, token, paramsMap);
            //break;
        }
        return null;
    }
     ...
     * @param paramsMap 请求参数
     */
    private void requestGetBySyn(String actionUrl, String token, HashMap<String, String> paramsMap) {
        ...
        try {
            ...
            //创建一个请求
            Request request = addHeaders(token).url(requestUrl).build();
            //创建一个Call
     ...
     * @param paramsMap 请求参数
     */
    private void requestPostBySyn(String actionUrl, String token, HashMap<String, String> paramsMap) {
        try {
     ...
     * @param paramsMap 请求参数
     */
    private String requestPostBySynWithForm(String actionUrl, String token, HashMap<String, String> paramsMap) {
        String result = "";
        try {
            ...
            //创建一个请求
            final Request request = addHeaders(token).url(requestUrl).post(formBody).build();
            //创建一个Call
            final Call call = mOkHttpClient.newCall(request);
            //执行请求
     ...
     * @param <T>         数据泛型
     **/
    public <T> Call requestAsyn(String actionUrl, String token, int requestType,
                                HashMap<String, String> paramsMap, ReqCallBack<T> callBack) {
        Call call = null;
        switch (requestType) {
            case TYPE_GET:
                call = requestGetByAsyn(actionUrl, token, paramsMap, callBack);
                break;
            case TYPE_POST_JSON:
                call = requestPostByAsyn(actionUrl, token, paramsMap, callBack);
                break;
            case TYPE_POST_FORM:
                call = requestPostByAsynWithForm(actionUrl, token, paramsMap, callBack);
                break;
        }
        return call;
    }
     ...
     * @return
     */
    private <T> Call requestGetByAsyn(String actionUrl, String token, HashMap<String, String> paramsMap, final ReqCallBack<T> callBack) {
        StringBuilder tempParams = new StringBuilder();
        try {
            int pos = 0;
            if(paramsMap!=null){
     ...
     * @return
     */
    private <T> Call requestPostByAsyn(String actionUrl, String token, HashMap<String, String> paramsMap, final ReqCallBack<T> callBack) {
        try {
            StringBuilder tempParams = new StringBuilder();
            int pos = 0;
            for (String key : paramsMap.keySet()) {
     ...
     * @return
     */
    private <T> Call requestPostByAsynWithForm(String actionUrl, String token, HashMap<String, String> paramsMap, final ReqCallBack<T> callBack) {
        try {
            FormBody.Builder builder = new FormBody.Builder();
            if(paramsMap!=null){
                for (String key : paramsMap.keySet()) {
