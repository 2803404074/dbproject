 ...
 * 描述:TODO 商品规格选择View
 */
public class ShoppingSelectWM extends LinearLayout {
    private boolean OK;
    ...
    private List<com.example.gangedrecyclerview.GoodsDetails.ProductInfoVoList> productInfo;
    /**
     * 数据源
     */
    private List<com.example.gangedrecyclerview.GoodsDetails.GoodsSpecVoList> list;
    /**
     * 上下文
     */
    ...
    public ShoppingSelectWM(Context context) {
        super(context);
        initView(context);
    }

    public ShoppingSelectWM(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }
    ...
    public void getView() {
        ...
        for (com.example.gangedrecyclerview.GoodsDetails.GoodsSpecVoList attr : list) {
            //设置规格分类的标题
            ...
            addView(textView);
            //设置一个大规格下的所有小规格
            FlowLayoutWM layout = new FlowLayoutWM(context, attr.getGoodsSpecList());
            ...
            if (null == attr.getGoodsSpecList()) return;
            for (int k = 0; k < attr.getGoodsSpecList().size(); k++) {
                com.example.gangedrecyclerview.GoodsDetails.GoodsSpecVoList.GoodsSpecList smallAttr = attr.getGoodsSpecList().get(k);
                //属性按钮
...
//                }
            ...
            if (null != productInfo) {
                for (int i = 0; i < productInfo.size(); i++) {
                    if (lastStr.equals(productInfo.get(i).getGoodsSpecIds())) {
                        if (!StringUtils.isEmpty(productInfo.get(i).getSellingPrice())) {
                            lastPrice = Double.valueOf(productInfo.get(i).getSellingPrice());
                        }
                        ...
                        //设置productId
                        productId = Integer.parseInt(TextUtil.isNull(productInfo.get(i).getId()));
    ...
    public void setData(List<com.example.gangedrecyclerview.GoodsDetails.GoodsSpecVoList> data) {
        list = data;
        getView();
    }
     ...
     */
//    public void setTextViewAndGGproject(int type, TextView number, TextView countPrice, TextView clike, TextView stock, List<com.example.gangedrecyclerview.GoodsDetails.ProductInfoVoList> productInfo) {
//        this.type = type;
//        this.number = number;
...
//        }
//    }
    public void setTextViewAndGGproject(int type, TextView number, TextView countPrice, TextView clike, List<com.example.gangedrecyclerview.GoodsDetails.ProductInfoVoList> productInfo) {
        this.type = type;
        this.number = number;
        this.countPrice = countPrice;
        this.productInfo = productInfo;
